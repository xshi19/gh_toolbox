function [mu,gamma,Sigma,lambda,chi,psi,llh] = ...
    gh_em(sample,mu0,gamma0,Sigma0,lambda0,chi0,psi0,maxiter,distr,fixt,thres)
...
for iter=1:maxiter 
    ...
    % M step
    % l_ = jgh_llh(mu,gamma,Sigma,lambda,chi,psi,S1,S2,S3,S4,S5,S6);
    mu = (S4-S2*S5)/(1-S1*S2);
    ...
    % set det(Sigma) = 1 in each step
    [mu,gamma,Sigma,lambda,chi,psi] = ...
        gh_reg(mu,gamma,Sigma,lambda,chi,psi,4);
    % l = jgh_llh(mu,gamma,Sigma,lambda,chi,psi,S1,S2,S3,S4,S5,S6);
    % [l_,l]
end
